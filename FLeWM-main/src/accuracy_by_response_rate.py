...
class Device:
    def __init__(self, device_id, participation_coefficient = 2.0, dataset=None):
        ...
        if dataset is None:
            dataset = self._generate_set(1, participation_coefficient=participation_coefficient)
    ...
    def _generate_large_train_set(self, num_rows, participation_coefficient=2.0):
        """
        This method generates a larger dataset for training purposes. NOTE: calling this method
        mutates the dataset attribute.
        """
        while True:
            large_dataset = self._generate_set(num_rows, participation_coefficient)
    ...
    def _generate_set(self, n_samples, participation_coefficient=2.0):
        ...
        pRS0 = expit(participation_coefficient * D1)
...
def run_missingness(num_rows, participation_coefficient):
    ...
    while len(filtered_devices) < num_rows:
        ...
        device = Device(i, participation_coefficient)

        device_survey = device.survey_local().reset_index()
        if device_survey["R"][0] == 1:
            filtered_devices.append(device)
            device._generate_large_train_set(1000, participation_coefficient)
    ...
    response_rate = len(filtered_devices) / len(devices)
    ...
    return response_rate, {demo : np.mean(accuracy_list) for demo, accuracy_list in accuracies.items()}

def run_missingness_with_correction(num_rows, participation_coefficient):
...
def create_plot(
    ...
    x_axis_label = "Overall Participation"
...
def run_simulation(
    manager, participation_coefficient, lock, results_missing, results_missing_with_correction, semaphore
):
    ...
    missing_response_rate, missing_res = run_missingness(5000, participation_coefficient)
    corrected_response_rate, missing_res_with_correction = run_missingness_with_correction(5000, participation_coefficient)

    def myround(x, base = 0.025):
        return base * round(x/base)

    missing_response_rate = myround(missing_response_rate, 0.025)
    corrected_response_rate = myround(corrected_response_rate, 0.025)

    lock.acquire()

    for demographic, accuracy in missing_res.items():
        if missing_response_rate not in results_missing:
            results_missing[missing_response_rate] = manager.dict()
        if demographic not in results_missing[missing_response_rate]:
            results_missing[missing_response_rate][demographic] = manager.list()
        
        results_missing[missing_response_rate][demographic].append(accuracy)

    for demographic, accuracy in missing_res_with_correction.items():
        if corrected_response_rate not in results_missing_with_correction:
            results_missing_with_correction[corrected_response_rate] = manager.dict()
        if demographic not in results_missing_with_correction[corrected_response_rate]:
            results_missing_with_correction[corrected_response_rate][demographic] = manager.list()
        
        results_missing_with_correction[corrected_response_rate][demographic].append(accuracy)
...
if __name__ == "__main__":
    ...
    image_id = random.randint(100_000_000, 1_000_000_000)
    ...
    num_rows_values = [0.1 * i for i in range(-80,80)]
