...
class Server:
        ...
        while not shutdown:
            try:
                ...
                if e.errno == errno.EMFILE:
                    ...
                    print(
                        f"WARN: Ran out of file descriptors ports. Slowing down train speed.",
                        flush=True,
                    )
                    sleep(3)
    ...
    def send_train_notification(self, clients):
        ...
        msg = pickle.dumps({"type": "TRAN", "data": None})
    ...
    def send_eval_notification(
        ...
        msg = pickle.dumps({"type": "EVAL", "data": dataset_to_use})
    ...
    def survey_devices(self):
        ...
        self.lock.acquire()
        clients_to_survey = self.device_addresses.values()
        self.lock.release()
...
def run_no_missingness(num_rows):
    sleep(15)
    server = Server("no-missing")

    server_thread = threading.Thread(target=server.serve)
    server_thread.start()

    devices = [
        Device(i, local_port=10000 + i) for i in tqdm(range(num_rows), disable=None)
    ]
    print(
        f"After survey, {len(server.participating_devices)} devices are participating and {len(server.missing_devices)} are missing"
    )
    ...
    # server.train(int(num_rows * NUM_EPOCHS / BATCH_SIZE), 0, 16)
    server.train(10, 0, 16)

    sleep(15)

    metrics = server.test(500)
...
def run_missingness(num_rows):
    ...
    devices = [
        Device(i, local_port=10000 + i) for i in tqdm(range(2 * num_rows), disable=None)
    ]  # We use 2x the number of rows to add more participating devices atm
...
if __name__ == "__main__":
    ...
    num_rows_values = [800 * i for i in range(1, 2)]
